...
export class EventsService {
    ...
    constructor(private chainId: number, private provider: providers.JsonRpcProvider, private logger: Logger, private reputationService: ReputationService, private mempoolService: MempoolService, private eventBus: ExecutorEventBus, private entryPointAddrs: string[], private db: IDbController) {
    ...
    async handlePastEvents(): Promise<void> {
        ...
        for (const contract of this.entryPoints) {
            ...
            if (events.length > 0) {
                ...
                if (!(this.lastBlockPerEntryPoint[address] ?? 0) || Number(this.lastBlockPerEntryPoint[address]) < blockNum) {
    ...
    async handleEvent(ev: ParsedEventType): Promise<void> {
        switch (ev.event) {
            ...
            case "SignatureAggregatorForUserOperations":
                await this.handleAggregatorChangedEvent(ev as SignatureAggregatorChangedEvent);
                break;
        }
    }

    async handleAggregatorChangedEvent(ev: SignatureAggregatorChangedEvent): Promise<void> {
    ...
    private async fetchLastBlockPerEntryPoints(): Promise<void> {
        const entry = await this.db.get<typeof this.lastBlockPerEntryPoint>(this.LAST_BLOCK_KEY).catch(() => null);
        if (entry) {
