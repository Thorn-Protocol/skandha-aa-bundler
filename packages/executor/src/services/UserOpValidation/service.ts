...
import { ExecutionResult, NetworkConfig, UserOpValidationResult } from "../../interfaces";
import { ReputationService } from "../ReputationService";
import { Skandha } from "../../modules";
import { EstimationService, SafeValidationService, UnsafeValidationService } from "./validators";
...
export class UserOpValidationService {
  ...
  async validateForEstimation(userOp: UserOperationStruct, entryPoint: string): Promise<ExecutionResult> {
  ...
  async validateForEstimationWithSignature(userOp: UserOperationStruct, entryPoint: string): Promise<UserOpValidationResult> {
    if (this.networkConfig.entryPointForwarder.length > 2) {
  ...
  async simulateValidation(userOp: UserOperationStruct, entryPoint: string, codehash?: string): Promise<UserOpValidationResult> {
  ...
  async binarySearchCGL(userOp: UserOperationStruct, entryPoint: string): Promise<UserOperationStruct> {
    if (this.config.unsafeMode) {
