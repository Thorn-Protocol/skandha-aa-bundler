import { defaultTopicScoreParams, PeerScoreParams, TopicScoreParams, PeerScoreThresholds } from "@chainsafe/libp2p-gossipsub/score";
...
const maxPositiveScore = (MAX_IN_MESH_SCORE + MAX_FIRST_MESSAGE_DELIVERIES_SCORE) * (BEACON_BLOCK_WEIGHT + +BEACON_AGGREGATE_PROOF_WEIGHT + beaconAttestationSubnetWeight * ATTESTATION_SUBNET_COUNT + VOLUNTARY_EXIT_WEIGHT + PROPOSER_SLASHING_WEIGHT);
 ...
 */
export function computeGossipPeerScoreParams({ config }: any): Partial<PeerScoreParams> {
...
function getAllTopicsScoreParams(precomputedParams: PreComputedParams): Record<string, TopicScoreParams> {
...
function getTopicScoreParams({ epochDurationMs, slotDurationMs, scoreParameterDecayFn }: PreComputedParams, { topicWeight, expectedMessageRate, firstMessageDecayTime, meshMessageInfo }: TopicScoreInput): TopicScoreParams {
    ...
    if (meshMessageInfo) {
        ...
        params.meshMessageDeliveriesWeight = (-1 * maxPositiveScore) / (params.topicWeight * Math.pow(params.meshMessageDeliveriesThreshold, 2));
